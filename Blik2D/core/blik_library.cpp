...
namespace BLIK
{
    class ApiFunctions
	{
			...
			: Type(type), FuncName(funcname), FuncFullName(funcfullname), Param(param)
		{
			...
			{
				if(blik_strcmp("void", typecode) == 0) return "void";
				else if(blik_strcmp("api_int", typecode) == 0) return "int";
				else if(blik_strcmp("api_uint", typecode) == 0) return "unsigned int";
				else if(blik_strcmp("api_flag", typecode) == 0) return "int";
				else if(blik_strcmp("api_enum", typecode) == 0) return "int";
				else if(blik_strcmp("api_long", typecode) == 0) return "long";
				else if(blik_strcmp("api_float", typecode) == 0) return "float";
				else if(blik_strcmp("api_object", typecode) == 0) return "Object";
				else if(blik_strcmp("api_string", typecode) == 0) return "String";
				else if(blik_strcmp("api_bytes", typecode) == 0) return "byte[]";
				else if(blik_strcmp("api_floats", typecode) == 0) return "float[]";
				else if(blik_strcmp("api_results", typecode) == 0) return "String";
				else BLIK_ASSERT("알 수 없는 typecode값으로 ApiFunctions구성에 실패하였습니다", false);
				return "unknown_type";
			};
			if(type) TypeForJava = ApiToJava(type);
			if(param && blik_strcmp("void", param) != 0)
			{
	...
	{
		blik_snprintf(temp, tempsize, "%u|%u", a, b);
		#if BLIK_ANDROID
	...
	{
		blik_snprintf(temp, tempsize, "%f|%f|%f", a, b, c);
		#if BLIK_ANDROID
			return ((JNIEnv*) env)->NewStringUTF(temp);
		#else
			return temp;
	...
	{
		blik_snprintf(temp, tempsize, "%f|%f|%f|%f", a, b, c, d);
		#if BLIK_ANDROID
			return ((JNIEnv*) env)->NewStringUTF(temp);
		#else
			return temp;
	...
	class ApiBuilder
	{
		...
		public: void WriteTypes(chars value)
		{
			...
			blik_snprintf(Temp, 1024,
				...
				"void Init%s(%s dll);\r\n", (chars) Package, value);
			String Text = Temp;
			blik_fwrite((chars) Text, 1, Text.Length(), FilePtr);
		}
		...
		public: void WriteImplement()
		{
			...
			while(CurFunction = CurFunction->Next)
			{
				...
				blik_snprintf(Temp, 1024,
					...
					"}\r\n",
					CurFunction->Type, CurFunction->FuncFullName, CurFunction->Param,
					(blik_strcmp("void", CurFunction->Type) == 0)? "" : "return ",
...
BLIK_API_FUNC(BLIK, void, PrintFunctions)()
{
	ApiFunctions* CurFunction = ApiFunctions::Top();
...
BLIK_API_FUNC(BLIK, void, BuildForMFC)(chars package)
{
	void* HeaderFile = blik_fopen(String(package) + ".h", "wb");
	ApiBuilder(HeaderFile, package).WriteTypes("HMODULE");
	ApiBuilder(HeaderFile, package).WriteDeclares();
	blik_fclose(HeaderFile);

	void* SourceFile = blik_fopen(String(package) + ".cpp", "wb");
	...
	blik_fclose(SourceFile);
}

BLIK_API_FUNC(BLIK, void, BuildForXCODE)(chars package)
{
	void* HeaderFile = blik_fopen(String(package) + ".h", "wb");
	...
	void* SourceFile = blik_fopen(String(package) + ".mm", "wb");
...
BLIK_API_FUNC(BLIK, void, BuildForJAVA)(chars package)
{
	...
	void* SourceFile = blik_fopen(ApplicationName + ".java", "wb");
