...
#if !BLIK_NDEBUG & BLIK_NEED_LOW_DEBUGING
...
namespace BLIK
{
    buffer BufferImpl::Alloc(BLIK_DBG_PRM sint32 count, const BufferSpec* spec, bool noConstructorOnce)
    {
        ...
        BLIK_ASSERT("메모리가 부족합니다", NewNaked);

        NewNaked[0] = count;
        NewNaked[1] = (sblock) spec;
        if(!noConstructorOnce)
            spec->Create(&NewNaked[2], count);
        BufferImpl::DebugAttach(BLIK_DBG_ARG NewNaked, BlockLength, RealLength);
    ...
    {
        BLIK_ASSERT("버퍼가 nullptr입니다", buf);
        const nakedbuffer Naked = (const nakedbuffer) &buf[-2];
        const BufferSpec* Spec = (const BufferSpec*) Naked[1];
        #if !BLIK_NDEBUG & BLIK_NEED_LOW_DEBUGING
            BLIK_ASSERT("버퍼가 손상되었습니다", Naked[0] == ~Naked[2 + CALC_BLOCK_LEN(Spec->SizeOf() * Naked[0])]);
        #endif
        return Naked;
    }
    ...
    void* BufferImpl::At(const buffer buf, sint32 index, chars castingName)
    {
        ...
        const BufferSpec* Spec = (const BufferSpec*) Naked[1];
        BLIK_ASSERT("허용범위를 초과한 인덱스입니다", 0 <= index && index < Naked[0]);
            ...
            if(Spec->NameOf() != castingName) // 동일한 타입은 명칭의 주소도 같음
            {
                ...
                BLIK_ASSERT("접근타입이 다르며 상속관계도 아닙니다", *ParentFocus == '\0');
            }
        #endif
    ...
    void BufferImpl::DebugAttach(BLIK_DBG_PRM const nakedbuffer naked, sint32 blockLen, sint32 realLen)
    {
        #if !BLIK_NDEBUG & BLIK_NEED_LOW_DEBUGING
            sblock* DebugInfo = &naked[2 + blockLen];
            DebugInfo[0] = ~naked[0]; // ~Count정보
            DebugInfo[1] = (sblock) realLen;
            DebugInfo[2] = (sblock) BLIK_DBG_FILE;
            DebugInfo[3] = (sblock) BLIK_DBG_LINE;
            DebugInfo[4] = (sblock) BLIK_DBG_FUNC;
    ...
    buffer Buffer::AllocBySample(BLIK_DBG_PRM sint32 count, const buffer sample)
    {
        const nakedbuffer SampleNaked = BufferImpl::Convert(sample);
        const BufferSpec* SampleSpec = (const BufferSpec*) SampleNaked[1];
        return BufferImpl::Alloc(BLIK_DBG_ARG count, SampleSpec, false);
    }

    buffer Buffer::Realloc(BLIK_DBG_PRM buffer buf, sint32 count)
    {
        const nakedbuffer OldNaked = BufferImpl::Convert(buf);
    ...
    buffer Buffer::Clone(BLIK_DBG_PRM const buffer src)
    {
        if(!src) return nullptr;
        const sint32 CopyCount = CountOf(src);

        buffer DstBuffer = AllocBySample(BLIK_DBG_ARG CopyCount, src);
    ...
    void Buffer::Copy(buffer dstBuf, sint32 dstOff, const buffer srcBuf, sint32 srcOff, sint32 count)
    {
        ...
        BLIK_ASSERT("서로 다른 타입간 복사입니다", DstNaked[1] == SrcNaked[1]);
        BLIK_ASSERT("복사범위가 dst를 초과합니다", 0 <= dstOff && dstOff + count <= DstNaked[0]);
        BLIK_ASSERT("복사범위가 src를 초과합니다", 0 <= srcOff && srcOff + count <= SrcNaked[0]);
