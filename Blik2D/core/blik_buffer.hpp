...
#define BLIK_FRIENDSHIP_FOR_BUFFER(TYPE) friend class BufferSpecPool<TYPE>;
#define BLIK_CONSTRUCTOR(PTR, IDX, TYPE, ...) new(BLIK_PTR_TO_SBLOCK(((buffer) PTR) + sizeof(TYPE) * (IDX))) TYPE(__VA_ARGS__)
#define BLIK_DECLARE_BUFFERED_CLASS(TYPE, PARENT, ...) \
    class TYPE : public PARENT {public: TYPE() : PARENT(__VA_ARGS__) {BLIK_ASSERT("사용금지", false);} ~TYPE() {} \
    TYPE& operator=(const TYPE& rhs) {BLIK_ASSERT("사용금지", false); return *this;}}
#if BLIK_MAC_OSX || BLIK_IPHONE || BLIK_X64
    #define BLIK_PTR_TO_SBLOCK(VALUE) ((sblock)(sint64) VALUE)
#else
    #define BLIK_PTR_TO_SBLOCK(VALUE) ((sblock) VALUE)
#endif
...
namespace BLIK
{
    ...
    {
    public:
        static buffer Alloc(BLIK_DBG_PRM sint32 count, const BufferSpec* spec, bool noConstructorOnce);
        ...
        static void DebugAttach(BLIK_DBG_PRM const nakedbuffer naked, sint32 blockLen, sint32 realLen);
    ...
    public:
        ...
        static chars Naming()
        {
            ...
            if(TypeName[0] == '\0')
            {
                ...
                BLIK_ASSERT("타입명이 너무 깁니다", TypeNameLength < 256);
    ...
    public:
        ...
        */
        static buffer Alloc(BLIK_DBG_PRM sint32 count)
        {return BufferImpl::Alloc(BLIK_DBG_ARG count, BufferSpecPool<uint08>::Summon(EnumToType<datatype_pod_canmemcpy>()), false);}
        ...
        */
        ...
        static buffer Alloc(BLIK_DBG_PRM sint32 count)
        {return BufferImpl::Alloc(BLIK_DBG_ARG count, BufferSpecPool<TYPE>::Summon(EnumToType<DATATYPE>()), false);}
        ...
        */
        template<typename TYPE, datatype DATATYPE = datatype_class_nomemcpy>
        static buffer AllocNoConstructorOnce(BLIK_DBG_PRM sint32 count)
        {return BufferImpl::Alloc(BLIK_DBG_ARG count, BufferSpecPool<TYPE>::Summon(EnumToType<DATATYPE>()), true);}
        ...
        */
        static buffer AllocBySample(BLIK_DBG_PRM sint32 count, const buffer sample);
        ...
        */
        static buffer Realloc(BLIK_DBG_PRM buffer buf, sint32 count);
        ...
        \see Copy
        */
        static buffer Clone(BLIK_DBG_PRM const buffer src);
