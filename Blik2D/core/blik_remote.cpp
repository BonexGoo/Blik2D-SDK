...
private:
    ...
    {
        BLIK_ASSERT("잘못된 시나리오입니다", !spec->m_localcall);
        ...
        for(sint32 i = 0; i < ParamCount; ++i)
        {
            ...
            {
                BLIK_ASSERT("송신할 수 없는 파라미터가 있습니다", false);
    ...
    uint08s* ResultToMessage(const String MethodName, id_cloned_share result)
    {
        ...
        {
            BLIK_ASSERT("송신할 수 없는 리턴값입니다", false);
    ...
    id_cloned_share MessageToCallOrReturn(bytes message, String& MethodName)
    {
                ...
                if(const Remote::Method** CurMethod = m_callMethods.Access(RefName))
                {
                    ...
                    for(sint32 i = 0; i < ParamCount; ++i)
                    {
                        ...
                        case 5: // StrParam
                            ...
                            break;
                        default:
                            BLIK_ASSERT("수신내용에 알 수 없는 파라미터가 있어 생략합니다", false);
                            break;
                        }
                    }
        ...
        case 5: // 리턴값 : StrParam
            ...
            break;
        default:
            BLIK_ASSERT("알 수 없는 수신내용입니다", false);
            break;
        }
        ...
        if(ReturnValue)
        {
            ...
            else BLIK_ASSERT("수신된 리턴값을 받을 함수가 없습니다", false);
        }
        return nullptr;
    }
    ...
    {
        BLIK_ASSERT("잘못된 호출입니다", m_isServer);
...
namespace BLIK
{
    ...
    {
        BLIK_ASSERT("복사대상과의 타입이 일치하지 않습니다", m_calling == rhs.m_calling);
        if(this != &rhs)
        {
            if(m_calling)
            {
            ...
            {
                BLIK_ASSERT("잘못된 시나리오입니다", !func.m_remote);
    ...
    {
        BLIK_ASSERT("잘못된 호출입니다", !m_calling);
    ...
    {
        BLIK_ASSERT("bytes타입의 length는 -1이 될 수 없습니다", length != -1);
        HexParam NewHex;
    ...
    {
        BLIK_ASSERT("value는 nullptr이 될 수 없습니다", value);
        if(m_calling)
        {
            AddParam(value);
            return *this;
    ...
    {
        BLIK_ASSERT("잘못된 시나리오입니다", !m_calling);
