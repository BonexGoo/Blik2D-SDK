...
#define BLIK_API_IMPL(CODE) \
    { \
        ReadyForPlatform(BLIK_API_ENV); \
        CODE; \
    }
#define BLIK_API_IMPL_RETURN(...) \
    { \
        ReadyForPlatform(BLIK_API_ENV); \
        return ReturnForPlatform(BLIK_API_ENV, __VA_ARGS__); \
    }
#define BLIK_API_IMPL_RESULTS(CODE, ...) \
    { \
        ReadyForPlatform(BLIK_API_ENV); \
        CODE; \
        static char temp[1024]; \
        return ResultsForPlatform(BLIK_API_ENV, temp, 1024, __VA_ARGS__); \
    }
#define BLIK_API_OBJ(OBJ) ObjectForPlatform(BLIK_API_ENV, OBJ)
#define BLIK_API_STR(STR) StringForPlatform(BLIK_API_ENV, STR)
#define BLIK_API_BRR(BRR) ByteArrayForPlatform(BLIK_API_ENV, BRR)
#define BLIK_API_FRR(FRR) FloatArrayForPlatform(BLIK_API_ENV, FRR)
