...
#include <platform/blik_platform_impl.hpp>
    ...
    public:
        cocos2d::Scene* CreateWithBlik()
        {
            BLIK_ASSERT("m_cb가 nullptr입니다", m_cb);
            cocos2d::Scene* NewScene = m_cb();
            BlikCreateCore(NewScene->getChildren());
            return NewScene;
        }

        static void ResizeWithBlik(cocos2d::Node* node, sint32 w, sint32 h)
        {
            node->setContentSize(cocos2d::Size(w, h));
            cocos2d::ui::Helper::doLayout(node);
            Platform::PassAllViews([](void* api, payload data)->bool
            {ResizeViewWithBlik((ViewAPI*) api); return false;}, nullptr);
        }

        static void ResizeViewWithBlik(ViewAPI* api)
        {
    ...
    private:
        void BlikCreateCore(const cocos2d::Vector<cocos2d::Node*>& children)
        {
            for(auto child : children)
            {
                ...
                else BlikCreateCore(child->getChildren());
            }
        }
    ...
    public:
        static CCHelper* Pool(chars scene, bool creatable)
        {
            ...
            BLIK_ASSERT("파라미터가 nullptr입니다", scene);
            ...
            if(m_api->hasViewManager())
            {
                if(ViewClass* View = m_api->getClass())
                {
                    ...
                    if(View->getResizingValue(w, h))
                    {
                        ...
                        CCHelper::ResizeViewWithBlik(m_api);
                    }
                }
                m_api->paint();
            }
    ...
    public:
        ...
        GenericView(ViewManager* manager, chars name, sint32 width, sint32 height)
        {
            ...
            BLIK_CONSTRUCTOR(NewAPI, 0, ViewAPI, ViewAPI::PT_GenericView, (buffer) this, manager, updater, this);
    ...
    public:
        ...
        {
            // 해당씬
            CCHelper::ResizeWithBlik(scene, ClientSize.w, ClientSize.h);
            ...
            for(sint32 i = 0, iend = childs.size(); i < iend; ++i)
            {
                ...
                CCHelper::ResizeWithBlik(curchild, ClientSize.w, ClientSize.h);
            }
        }
        ...
        virtual bool applicationDidFinishLaunching()
        {
                        ...
                        if(code == HC_ACTION)
                        {
                            ...
                            if(pmsg->hwnd == s_win_handle())
                            {
                                if(wParam == WM_MOUSEMOVE || wParam == WM_LBUTTONUP)
                                {
                                    ...
                                    BLIK::Platform::PassAllViews([](void* api, payload data)->bool
                                    {
            ...
            // Intro씬 탑재
            cocos2d::Scene* scene = CCHelper::Intro()->CreateWithBlik();
