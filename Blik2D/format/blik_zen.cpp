...
namespace BLIK
{
    const Zen& Zen::operator[](sint32 index) const
    {
        ...
        {
            BLIK_ASSERT("데이터에 심각한 오류가 있습니다", m_iIndexer != jumper_null);
            const jumper* CurIndexer = (const jumper*) &JumperToPtr(m_iIndexer)[m_jumpbase];

            BLIK_ASSERT("데이터에 심각한 오류가 있습니다", CurIndexer[index] != jumper_null);
    ...
    const Zen& Zen::operator()(sint32 index, chararray* getname) const
    {
        ...
        {
            BLIK_ASSERT("데이터에 심각한 오류가 있습니다", m_nIndexer != jumper_null);
            const NamableIndexer& CurIndexer = ((const NamableIndexer*) &JumperToPtr(m_nIndexer)[m_jumpbase])[index];

            BLIK_ASSERT("데이터에 심각한 오류가 있습니다", CurIndexer.Finder != jumper_null);
            ...
            if(getname)
            {
                ...
                *CurWord = '\0';
                ...
                BLIK_ASSERT("데이터에 심각한 오류가 있습니다", CurWord == &Collector.At(0));
                *getname = Collector;
            }

            BLIK_ASSERT("데이터에 심각한 오류가 있습니다", CurFinder->ZenData != jumper_null);
    ...
    const Zen& Zen::operator()(chars name) const
    {
        ...
        while(*name)
        {
            ...
            BLIK_ASSERT("데이터에 심각한 오류가 있습니다", CurFinder->Word == *name);
            name++;
        }
    ...
    jumper Zen::BuildCore(const Property& data, sint32s& col, Map<sint32>& map)
    {
            ...
            for(sint32 i = 0, iend = NewFinderPool.Count(); i < iend; ++i)
            {
                ...
                for(sint32 src = CurFinder.ChildFirst, dst = 0; src <= CurFinder.ChildLast; ++src, ++dst)
                {
                    if(CurFinder.Childs[src] != jumper16_null)
                    {
                        ...
                        BLIK_ASSERT("Jumper16의 범위를 벗어납니다",
                ...
                // Finder쓰기
                BLIK_ASSERT("인코딩에 실패하였습니다", NFinderPosPool[i] == col.Count());
        ...
        else
        {
            ...
            // String쓰기
            const sint32 CurStringSize = (sint32) blik_strlen(CurString);
