...
#if !defined(BLIK_NEED_ADDON_JPG) || (BLIK_NEED_ADDON_JPG != 0 && BLIK_NEED_ADDON_JPG != 1)
    #error BLIK_NEED_ADDON_JPG macro is invalid use
#endif
bool __LINK_ADDON_JPG__() {return true;} // 링크옵션 /OPT:NOREF가 안되서 임시코드

#if BLIK_NEED_ADDON_JPG

#include "blik_addon_jpg.hpp"
...
namespace BLIK
{
    id_jpg Customized_AddOn_Jpg_Create(id_bitmap_read bmp, sint32 quality)
    {
        ...
        buffer NewJpg = Buffer::Alloc(BLIK_DBG JpgBuffer.Count());
...
{
    BLIK_ASSERT(String::Format("jpeg인코딩에 실패하였습니다(message:%s)", message), false);
}
...
bool JPEGDecoder::decompress(unsigned int* dest)
{
    while (dec.output_scanline < dec.output_height) {
        ...
        {
        case JCS_RGB:
            BLIK_ASSERT(String::Format("JCS_RGB이면서 components는 %d인 예외상황입니다", dec.output_components),
            ...
            break;
        case JCS_GRAYSCALE:
            BLIK_ASSERT(String::Format("JCS_GRAYSCALE이면서 components는 %d인 예외상황입니다", dec.output_components),
            ...
            break;
        case JCS_CMYK:
            BLIK_ASSERT(String::Format("JCS_CMYK이면서 components는 %d인 예외상황입니다", dec.output_components),
...
{
    BLIK_ASSERT(String::Format("jpeg디코딩에 실패하였습니다(message:%s)", message), false);
}
