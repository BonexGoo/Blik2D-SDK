...
#if !defined(BLIK_NEED_ADDON_GIT) || (BLIK_NEED_ADDON_GIT != 0 && BLIK_NEED_ADDON_GIT != 1)
    #error BLIK_NEED_ADDON_GIT macro is invalid use
#endif
...
#include <addon/blik_integration_libgit2-0.22.2.h>

#include <blik.hpp>
#include "blik_addon_git.hpp"

extern "C"
{
    #undef BLIK_LIBGIT2_U_common_h
    #define BLIK_LIBGIT2_U_common_h "common.h"
    #include <addon/libgit2-0.22.2_for_blik/include/git2.h>
    ...
    static inline void Git_ErrorCheck(sint32 code)
    {
        ...
        BLIK_ASSERT(String::Format("GIT(%s/%d)", (chars) ErrorMessage, code), 0 <= code);
    }
    ...
    {
        ((GitClass*) data)->Progress(BLIK::PT_DownloadPack, String::Format("{\"info\"=\"%.*s\"}", length - 1, string));
        return 0;
    }
    ...
    static sint32 Git_Progress_DownloadTip(chars refname, const git_oid* oldid, const git_oid* newid, payload data)
    {
        ...
        if(git_oid_iszero(oldid))
            ((GitClass*) data)->Progress(BLIK::PT_DownloadTip,
    ...
    {
        ((GitClass*) data)->Progress(BLIK::PT_InflateFile,
...
namespace BLIK
{
    BLIK_DECLARE_ADDON_FUNCTION(Git, Create, id_git, chars, chars, chars, chars)
    BLIK_DECLARE_ADDON_FUNCTION(Git, Release, void, id_git)
    BLIK_DECLARE_ADDON_FUNCTION(Git, Update, void, id_git, AddOn::Git::ProgressCB, payload)
...
namespace BLIK
{
    ...
    {
        GitClass* NewGit = (GitClass*) Buffer::Alloc<GitClass>(BLIK_DBG 1);
