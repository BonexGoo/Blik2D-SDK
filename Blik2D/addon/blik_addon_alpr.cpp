...
#if !defined(BLIK_NEED_ADDON_ALPR) || (BLIK_NEED_ADDON_ALPR != 0 && BLIK_NEED_ADDON_ALPR != 1)
    #error BLIK_NEED_ADDON_ALPR macro is invalid use
#endif
...
#include <addon/openalpr-2.3.0_for_blik/src/openalpr/alpr.h>
#include <addon/opencv-3.1.0_for_blik/include/opencv2/opencv.hpp>

#include <blik.hpp>
#include "blik_addon_alpr.hpp"
...
namespace BLIK
{
	BLIK_DECLARE_ADDON_FUNCTION(Alpr, Create, id_alpr, chars, h_view)
	BLIK_DECLARE_ADDON_FUNCTION(Alpr, Release, void, id_alpr)
	BLIK_DECLARE_ADDON_FUNCTION(Alpr, Recognize, chars, id_alpr, id_bitmap_read)
	BLIK_DECLARE_ADDON_FUNCTION(Alpr, Summary, chars, chars)
...
namespace BLIK
{
    ...
    {
		BLIK_ASSERT("country는 nullptr가 될 수 없습니다", country);
        m_listener = listener;
    ...
    {
		AlprClass* NewAlpr = (AlprClass*) Buffer::Alloc<AlprClass>(BLIK_DBG 1);
	...
	chars Customized_AddOn_Alpr_Recognize(id_alpr alpr, id_bitmap_read bitmap)
	{
		...
		BLIK_ASSERT("bitmap은 nullptr가 될 수 없습니다", bitmap);
