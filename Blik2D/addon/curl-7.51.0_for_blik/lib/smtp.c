...
#include BLIK_CURL_U_smtp_h //original-code:"smtp.h"
...
#include BLIK_CURL_U_curl_gethostname_h //original-code:"curl_gethostname.h"
 ...
 */
static void state_smtp_BLIK(struct connectdata *conn, smtpstate newstate)
{
  struct smtp_conn *smtpc = &conn->proto.smtpc;
...
static CURLcode smtp_perform_ehlo(struct connectdata *conn)
{
  ...
  if(!result)
    state_smtp_BLIK(conn, SMTP_EHLO);
...
static CURLcode smtp_perform_helo(struct connectdata *conn)
{
  ...
  if(!result)
    state_smtp_BLIK(conn, SMTP_HELO);
...
static CURLcode smtp_perform_starttls(struct connectdata *conn)
{
  ...
  if(!result)
    state_smtp_BLIK(conn, SMTP_STARTTLS);
...
static CURLcode smtp_perform_upgrade_tls(struct connectdata *conn)
{
  ...
  if(!result) {
    if(smtpc->state != SMTP_UPGRADETLS)
      state_smtp_BLIK(conn, SMTP_UPGRADETLS);
...
static CURLcode smtp_perform_authentication(struct connectdata *conn)
{
  ...
  if(!smtpc->auth_supported ||
    ...
    state_smtp_BLIK(conn, SMTP_STOP);
    return result;
  }
  ...
  if(!result) {
    if(progress == SASL_INPROGRESS)
      state_smtp_BLIK(conn, SMTP_AUTH);
    else {
...
static CURLcode smtp_perform_command(struct connectdata *conn)
{
  ...
  if(!result)
    state_smtp_BLIK(conn, SMTP_COMMAND);
...
static CURLcode smtp_perform_mail(struct connectdata *conn)
{
  ...
  if(!result)
    state_smtp_BLIK(conn, SMTP_MAIL);
...
static CURLcode smtp_perform_rcpt_to(struct connectdata *conn)
{
  ...
  if(smtp->rcpt->data[0] == '<')
    ...
    result = Curl_pp_sendf(&conn->proto.smtpc.pp, "RCPT TO:<%s>",
                           smtp->rcpt->data);
  if(!result)
    state_smtp_BLIK(conn, SMTP_RCPT);
...
static CURLcode smtp_perform_quit(struct connectdata *conn)
{
  ...
  if(!result)
    state_smtp_BLIK(conn, SMTP_QUIT);
                                     ...
                                     smtpstate instate)
{
  ...
  if(!result)
    switch(progress) {
    case SASL_DONE:
      state_smtp_BLIK(conn, SMTP_STOP);  /* Authenticated */
      break;
                                     ...
                                     smtpstate instate)
{
  ...
  if(smtpcode/100 != 2) {
    ...
    if(smtp->rcpt)
      ...
      if(!result)
        state_smtp_BLIK(conn, SMTP_DATA);
    }
  }
...
static CURLcode smtp_connect(struct connectdata *conn, bool *done)
{
  ...
  *done = FALSE; /* default to not done yet */
  ...
  state_smtp_BLIK(conn, SMTP_SERVERGREET);
                          ...
                          bool premature)
{
  ...
  if(status) {
    ...
    state_smtp_BLIK(conn, SMTP_POSTDATA);
