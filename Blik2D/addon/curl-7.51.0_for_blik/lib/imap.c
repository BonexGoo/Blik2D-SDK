...
#include BLIK_CURL_U_http_h //original-code:"http.h" /* for HTTP proxy tunnel stuff */
#include BLIK_CURL_U_socks_h //original-code:"socks.h"
#include BLIK_CURL_U_imap_h //original-code:"imap.h"
#include BLIK_CURL_U_strtoofft_h //original-code:"strtoofft.h"
...
#include BLIK_CURL_U_strcase_h //original-code:"strcase.h"
#include BLIK_CURL_U_curl_sasl_h //original-code:"curl_sasl.h"
 ...
 */
static void state_imap_BLIK(struct connectdata *conn, imapstate newstate)
{
  struct imap_conn *imapc = &conn->proto.imapc;
...
static CURLcode imap_perform_capability(struct connectdata *conn)
{
  ...
  if(!result)
    state_imap_BLIK(conn, IMAP_CAPABILITY);
...
static CURLcode imap_perform_starttls(struct connectdata *conn)
{
  ...
  if(!result)
    state_imap_BLIK(conn, IMAP_STARTTLS);
...
static CURLcode imap_perform_upgrade_tls(struct connectdata *conn)
{
  ...
  if(!result) {
    if(imapc->state != IMAP_UPGRADETLS)
      state_imap_BLIK(conn, IMAP_UPGRADETLS);
...
static CURLcode imap_perform_login(struct connectdata *conn)
{
  ...
  if(!conn->bits.user_passwd) {
    state_imap_BLIK(conn, IMAP_STOP);
  ...
  if(!result)
    state_imap_BLIK(conn, IMAP_LOGIN);
...
static CURLcode imap_perform_authentication(struct connectdata *conn)
{
  ...
  if(!result) {
    if(progress == SASL_INPROGRESS)
      state_imap_BLIK(conn, IMAP_AUTHENTICATE);
...
static CURLcode imap_perform_list(struct connectdata *conn)
{
  ...
  if(!result)
    state_imap_BLIK(conn, IMAP_LIST);
...
static CURLcode imap_perform_select(struct connectdata *conn)
{
  ...
  if(!result)
    state_imap_BLIK(conn, IMAP_SELECT);
...
static CURLcode imap_perform_fetch(struct connectdata *conn)
{
  ...
  if(!result)
    state_imap_BLIK(conn, IMAP_FETCH);
...
static CURLcode imap_perform_append(struct connectdata *conn)
{
  ...
  if(!result)
    state_imap_BLIK(conn, IMAP_APPEND);
...
static CURLcode imap_perform_search(struct connectdata *conn)
{
  ...
  if(!result)
    state_imap_BLIK(conn, IMAP_SEARCH);
...
static CURLcode imap_perform_logout(struct connectdata *conn)
{
  ...
  if(!result)
    state_imap_BLIK(conn, IMAP_LOGOUT);
                                     ...
                                     imapstate instate)
{
  ...
  if(!result)
    switch(progress) {
    case SASL_DONE:
      state_imap_BLIK(conn, IMAP_STOP);  /* Authenticated */
      break;
...
static CURLcode imap_connect(struct connectdata *conn, bool *done)
{
  ...
  *done = FALSE; /* default to not done yet */
  ...
  state_imap_BLIK(conn, IMAP_SERVERGREET);
                          ...
                          bool premature)
{
  ...
  if(status) {
    ...
    if(!data->set.upload)
      state_imap_BLIK(conn, IMAP_FETCH_FINAL);
    else {
      /* End the APPEND command first by sending an empty line */
      result = Curl_pp_sendf(&conn->proto.imapc.pp, "%s", "");
      if(!result)
        state_imap_BLIK(conn, IMAP_APPEND_FINAL);
    }
