 ...
 */
static void state_pop3_BLIK(struct connectdata *conn, pop3state newstate)
{
  struct pop3_conn *pop3c = &conn->proto.pop3c;
...
static CURLcode pop3_perform_capa(struct connectdata *conn)
{
  ...
  if(!result)
    state_pop3_BLIK(conn, POP3_CAPA);
...
static CURLcode pop3_perform_starttls(struct connectdata *conn)
{
  ...
  if(!result)
    state_pop3_BLIK(conn, POP3_STARTTLS);
...
static CURLcode pop3_perform_upgrade_tls(struct connectdata *conn)
{
  ...
  if(!result) {
    if(pop3c->state != POP3_UPGRADETLS)
      state_pop3_BLIK(conn, POP3_UPGRADETLS);
...
static CURLcode pop3_perform_user(struct connectdata *conn)
{
  ...
  if(!conn->bits.user_passwd) {
    state_pop3_BLIK(conn, POP3_STOP);
  ...
  result = Curl_pp_sendf(&conn->proto.pop3c.pp, "USER %s",
                         conn->user ? conn->user : "");
  if(!result)
    state_pop3_BLIK(conn, POP3_USER);
...
static CURLcode pop3_perform_apop(struct connectdata *conn)
{
  ...
  if(!result)
    state_pop3_BLIK(conn, POP3_APOP);
  ...
  if(pop3c->authtypes & pop3c->preftype & POP3_TYPE_SASL) {
    ...
    if(!result)
      if(progress == SASL_INPROGRESS)
        state_pop3_BLIK(conn, POP3_AUTH);
  }
...
static CURLcode pop3_perform_command(struct connectdata *conn)
{
  ...
  if(!result)
    state_pop3_BLIK(conn, POP3_COMMAND);
...
static CURLcode pop3_perform_quit(struct connectdata *conn)
{
  ...
  if(!result)
    state_pop3_BLIK(conn, POP3_QUIT);
                                     ...
                                     pop3state instate)
{
  ...
  if(!result)
    switch(progress) {
    case SASL_DONE:
      state_pop3_BLIK(conn, POP3_STOP);  /* Authenticated */
      break;
                                     ...
                                     pop3state instate)
{
  ...
  if(!result)
    state_pop3_BLIK(conn, POP3_PASS);
...
static CURLcode pop3_connect(struct connectdata *conn, bool *done)
{
  ...
  *done = FALSE; /* default to not done yet */
  ...
  state_pop3_BLIK(conn, POP3_SERVERGREET);
