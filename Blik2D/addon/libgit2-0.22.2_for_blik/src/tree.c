...
static bool valid_filemode_tree(const int filemode) //oz
{
...
static int append_entry_tree( //oz
	git_treebuilder *bld,
	const char *filename,
	const git_oid *id,
	...
	size_t start)
{
	 ...
	 */
		...
		if (next_slash) {
			...
			written = write_tree_tree(&sub_oid, repo, index, subdir, i); //oz
			if (written < 0) {
				git__free(subdir);
				goto on_error;
			} else {
			...
			error = append_entry_tree(bld, last_comp, &sub_oid, S_IFDIR); //oz
			git__free(subdir);
			if (error < 0)
				goto on_error;
		} else {
			error = append_entry_tree(bld, filename, &entry->id, entry->mode); //oz
			if (error < 0)
				goto on_error;
		}
	}
	...
	git_oid *oid, git_index *index, git_repository *repo)
{
	...
	ret = write_tree_tree(oid, repo, index, "", 0); //oz
	...
	git_filemode_t filemode)
{
	...
	if (!valid_filemode_tree(filemode)) //oz
