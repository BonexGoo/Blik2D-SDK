...
#include BLIK_LIBGIT2_V_zlib_h //original-code:<zlib.h>
#include BLIK_LIBGIT2_U_git2__object_h //original-code:"git2/object.h"
#include BLIK_LIBGIT2_U_git2__sys__odb_backend_h //original-code:"git2/sys/odb_backend.h"
#include "fileops.h"
...
static int foreach_cb_odbloose(void *_state, git_buf *path) //oz
{
	struct foreach_state *state = (struct foreach_state *) _state;
...
static int loose_backend__foreach(git_odb_backend *_backend, git_odb_foreach_cb cb, void *data)
{
	...
	error = git_path_direach(&buf, 0, foreach_cb_odbloose, &state); //oz
