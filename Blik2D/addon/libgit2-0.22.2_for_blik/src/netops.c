...
#include BLIK_LIBGIT2_U_http_parser_h //original-code:"http_parser.h"
...
static const char *prefix_http_netops = "http://"; //oz
static const char *prefix_https_netops = "https://"; //oz
		...
		const char *service_suffix)
{
	...
	if (!git__prefixcmp(url, prefix_http_netops)) { //oz
		path_search_start = url + strlen(prefix_http_netops); //oz
		default_port = "80";
	...
	} else if (!git__prefixcmp(url, prefix_https_netops)) { //oz
		path_search_start = url + strlen(prefix_https_netops); //oz
		default_port = "443";
		data->use_ssl = true;
	} else if (url[0] == '/')
