...
static int peel_error_object(int error, const git_oid *oid, git_otype type) //oz
{
	...
	git_otype target_type)
{
	...
	if ((error = check_type_combination(git_object_type(object), target_type)) < 0)
		return peel_error_object(error, git_object_id(object), target_type); //oz
	...
	if (error)
		error = peel_error_object(error, git_object_id(object), target_type); //oz
