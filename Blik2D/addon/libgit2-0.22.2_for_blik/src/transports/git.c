...
#include BLIK_LIBGIT2_U_git2_h //original-code:"git2.h"
#include BLIK_LIBGIT2_U_buffer_h //original-code:"buffer.h"
#include BLIK_LIBGIT2_U_netops_h //original-code:"netops.h"
#include BLIK_LIBGIT2_U_git2__sys__transport_h //original-code:"git2/sys/transport.h"
#include BLIK_LIBGIT2_U_stream_h //original-code:"stream.h"
#include BLIK_LIBGIT2_U_socket_stream_h //original-code:"socket_stream.h"

#undef OWNING_SUBTRANSPORT //oz
#define OWNING_SUBTRANSPORT(s) ((git_subtransport *)(s)->parent.subtransport)

static const char prefix_git[] = "git://";
static const char cmd_uploadpack_git[] = "git-upload-pack"; //oz
static const char cmd_receivepack_git[] = "git-receive-pack"; //oz
 ...
 */
static int gen_proto_git(git_buf *request, const char *cmd, const char *url) //oz
{
	char *delim, *repo;
	char host[] = "host=";
	size_t len;
...
static int send_command_git(git_proto_stream *s) //oz
{
	int error;
	git_buf request = GIT_BUF_INIT;

	error = gen_proto_git(&request, s->cmd, s->url); //oz
	if (error < 0)
		goto cleanup;
	...
	size_t *bytes_read)
{
	...
	*bytes_read = 0;
	...
	if (!s->sent_command && (error = send_command_git(s)) < 0) //oz
		return error;
	...
	git_smart_subtransport_stream **stream)
{
	...
	*stream = NULL;
	...
	error = git_proto_stream_alloc(t, stream_url, cmd_uploadpack_git, host, port, stream);
	...
	git_smart_subtransport_stream **stream)
{
	...
	*stream = NULL;
	...
	error = git_proto_stream_alloc(t, stream_url, cmd_receivepack_git, host, port, stream); //oz
