...
#ifdef GIT_SSH
#include BLIK_LIBSSH2_V_libssh2_h //original-code:<libssh2.h>
#endif
...
static const char cmd_uploadpack_ssh[] = "git-upload-pack"; //oz
static const char cmd_receivepack_ssh[] = "git-receive-pack"; //oz
 ...
 */
static int gen_proto_ssh(git_buf *request, const char *cmd, const char *url) //oz
{
	char *repo;
	int len;
...
static int send_command_ssh(ssh_stream *s) //oz
{
	int error;
	git_buf request = GIT_BUF_INIT;

	error = gen_proto_ssh(&request, s->cmd, s->url); //oz
	if (error < 0)
		goto cleanup;
	...
	size_t *bytes_read)
{
	...
	*bytes_read = 0;
	...
	if (!s->sent_command && send_command_ssh(s) < 0) //oz
		return -1;
...
static int ssh_uploadpack_ls(
	...
	git_smart_subtransport_stream **stream)
{
	const char *cmd = t->cmd_uploadpack ? t->cmd_uploadpack : cmd_uploadpack_ssh; //oz
...
static int ssh_receivepack_ls(
	...
	git_smart_subtransport_stream **stream)
{
	const char *cmd = t->cmd_receivepack ? t->cmd_receivepack : cmd_receivepack_ssh; //oz
