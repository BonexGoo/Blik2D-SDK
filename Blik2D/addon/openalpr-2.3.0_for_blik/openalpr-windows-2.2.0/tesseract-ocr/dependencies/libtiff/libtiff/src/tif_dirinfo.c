 ...
 */
#include BLIK_OPENALPR_U_tiffiop_h //original-code:"tiffiop.h"
#include <stdlib.h>
#include <string.h>
		...
		for (i = 0; i < tif->tif_nfields; i++) 
		{
			...
			if (fld->field_bit == FIELD_CUSTOM && 
				strncmp("Tag ", fld->field_name, 4) == 0) {
                                        _TIFFfree((tdata_t) fld->field_name); // modified by BLIK, original-code: _TIFFfree(fld->field_name);
					_TIFFfree(fld);
				}
		}   
...
_TIFFFindFieldInfoByName(TIFF* tif, const char *field_name, TIFFDataType dt)
{
	...
	/* NB: use sorted search (e.g. binary search) */
        ...
        ret = (const TIFFFieldInfo **) lfind(&pkey,
					     tif->tif_fieldinfo, 
					     (unsigned int*) &tif->tif_nfields, // modified by BLIK, ¿øº»(&tif->tif_nfields,)
					     sizeof(TIFFFieldInfo *),
					     tagNameCompare);
	return tif->tif_foundfield = (ret ? *ret : NULL);
}
...
_TIFFCreateAnonFieldInfo(TIFF *tif, ttag_t tag, TIFFDataType field_type)
{
	 ...
	 */
        sprintf((char*) fld->field_name, "Tag %d", (int) tag); // modified by BLIK, original-code: sprintf(fld->field_name, "Tag %d", (int) tag);
