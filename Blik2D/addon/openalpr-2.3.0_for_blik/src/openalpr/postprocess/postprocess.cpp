...
namespace alpr
{
  ...
  void PostProcess::insertLetter(string letter, int line_index, int charposition, float score)
  {
    ...
    for (int i = 0; i < letters[charposition].size(); i++)
    {
      if (letters[charposition][i].letter == letter &&
          // BLIK: 잘못된 라인개념 제거, 원본(letters[charposition][i].line_index == line_index &&)
    ...
    {
      Letter newLetter;
      // BLIK: 잘못된 라인개념 제거, 원본(newLetter.line_index = line_index;)
    ...
    if (this->config->debugPostProcess)
    {
      ...
      for (int i = 0; i < letters.size(); i++)
      {
        for (int j = 0; j < letters[i].size(); j++)
          // BLIK: 잘못된 라인개념 제거, 원본(cout << "PostProcess Line " << letters[i][j].line_index << " Letter: " << letters[i][j].charposition << " " << letters[i][j].letter << " -- score: " << letters[i][j].totalscore << " -- occurrences: " << letters[i][j].occurrences << endl;)
          // BLIK: 아래와 같이 수정!
          cout << "PostProcess Letter: " << letters[i][j].charposition << " " << letters[i][j].letter << " -- score: " << letters[i][j].totalscore << " -- occurrences: " << letters[i][j].occurrences << endl;
      }
    }
  ...
  bool PostProcess::analyzePermutation(vector<int> letterIndices, string templateregion, int topn)
  {
    ...
    // BLIK: 잘못된 라인개념 제거, 원본(int last_line = 0;)
    for (int i = 0; i < letters.size(); i++)
    {
      if (letters[i].size() == 0)
        continue;
      ...
      // Add a "\n" on new lines
      // BLIK: 잘못된 라인개념 제거, 원본(if (letter.line_index != last_line))
      // BLIK: 잘못된 라인개념 제거, 원본({)
      // BLIK: 잘못된 라인개념 제거, 원본(  possibility.letters = possibility.letters + "\n";)
      // BLIK: 잘못된 라인개념 제거, 원본(})
      // BLIK: 잘못된 라인개념 제거, 원본(last_line = letter.line_index;)

      // BLIK: 스킵된 문자도 표시하는게 타당함, 원본(if (letter.letter != SKIP_CHAR)
      {
