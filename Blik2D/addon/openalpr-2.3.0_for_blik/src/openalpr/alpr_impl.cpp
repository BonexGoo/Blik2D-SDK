...
namespace alpr
{
  ...
  AlprFullDetails AlprImpl::analyzeSingleCountry(cv::Mat colorImg, cv::Mat grayImg, std::vector<cv::Rect> warpedRegionsOfInterest)
  {
    ...
    while(!plateQueue.empty())
    {
	  ...
	  {
		  AlprDebug_AddRect(plateRegion.rect.x, plateRegion.rect.y,
			  plateRegion.rect.width, plateRegion.rect.height);
		  AlprDebug_Flush("disqualify", (config->country + ":" + pipeline_data.disqualify_reason).c_str());
	  }
	  else
	  {
		  AlprDebug_AddRect(plateRegion.rect.x, plateRegion.rect.y,
			  plateRegion.rect.width, plateRegion.rect.height);
		  AlprDebug_Flush("qualify", (config->country + ":Recognized").c_str());
	  }
	  // BLIK End
  ...
  cJSON* AlprImpl::createJsonObj(const AlprPlateResult* result)
  {
	...
	// add by BLIK, Begin
	cJSON *char_coords;
	cJSON_AddItemToObject(root, "char_coordinates_for_blik", char_coords=cJSON_CreateArray());
	for(int i = 0, iend = result->bestPlate.character_details.size(); i < iend; ++i)
	{
		cJSON *coords_object;
		coords_object = cJSON_CreateObject();
		AlprChar details = result->bestPlate.character_details[i];
		cJSON_AddNumberToObject(coords_object, "x0",  details.corners[0].x);
		cJSON_AddNumberToObject(coords_object, "y0",  details.corners[0].y);
		cJSON_AddNumberToObject(coords_object, "x1",  details.corners[1].x);
		cJSON_AddNumberToObject(coords_object, "y1",  details.corners[1].y);
		cJSON_AddNumberToObject(coords_object, "x2",  details.corners[2].x);
		cJSON_AddNumberToObject(coords_object, "y2",  details.corners[2].y);
		cJSON_AddNumberToObject(coords_object, "x3",  details.corners[3].x);
		cJSON_AddNumberToObject(coords_object, "y3",  details.corners[3].y);
		cJSON_AddItemToArray(char_coords, coords_object);
	}
	// add by BLIK, End
