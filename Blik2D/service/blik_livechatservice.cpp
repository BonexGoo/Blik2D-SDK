...
namespace BLIK
{
    ...
    {
        BLIK_DECLARE_NONCOPYABLE_CLASS(LiveChatCollector)
    ...
    private: // 전용객체
        class Manager
        {
            BLIK_DECLARE_NONCOPYABLE_CLASS(Manager)
        protected:
        ...
        class Payload
        {
            BLIK_DECLARE_NONCOPYABLE_CLASS(Payload)
        public:
    ...
    public:
        ...
        {
            BLIK_ASSERT("초기화는 한번만 할 수 있습니다", !mTasking);
            mService = service;
            mId = id;
            mMessageCb = cb;
            mMessageData = data;

            buffer NewPayload = Buffer::AllocNoConstructorOnce<Payload>(BLIK_DBG 1);
            BLIK_CONSTRUCTOR(NewPayload, 0, Payload, this);
    ...
    protected:
        static sint32 OnGoogleTask(buffer& self, Queue<buffer>& query, Queue<buffer>& answer, id_common common)
        {
            ...
            {
                BLIK_DECLARE_NONCOPYABLE_CLASS(GoogleManager)
            public:
                GoogleManager() {}
                ~GoogleManager() {}
            public:
                static Manager* Create() {return (Manager*) Buffer::Alloc<GoogleManager>(BLIK_DBG 1);}
    ...
    class LiveChatData
    {
        BLIK_DECLARE_NONCOPYABLE_CLASS(LiveChatData)
    ...
    public:
        class UserInfo
        {
            BLIK_DECLARE_NONCOPYABLE_CLASS(UserInfo)
        public:
            UserInfo() {}
            ~UserInfo() {}
        public:
    ...
    {
        BLIK_ASSERT("message가 nullptr입니다", message);
