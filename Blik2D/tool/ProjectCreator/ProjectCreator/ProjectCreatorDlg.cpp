...
static buffer ReplaceContent(buffer data, chars oldtext, chars newtext)
{
    ...
    const sint32 OldTextSize = blik_strlen(oldtext);
    const sint32 NewTextSize = blik_strlen(newtext);
    ...
    {
        if(Binary[i] == oldtext[0] && !blik_strncmp(&Binary[i], oldtext, OldTextSize))
        {
            if(iOld < i)
            {
